// Sanity-check variant: use SHORT user-style prompts (the kind a real user
// would type) instead of the honest-prompt-rewrites that match each ref.
// This guards against overfitting the test set.
...
const OUT_DIR = 'scripts/iter-gens-user'
const RUNS = parseInt(process.argv[2] ?? '3', 10)

const PROMPTS = [
  { ref: 'e31356d8346b58c0', label: 'pink_pastel',  prompt: 'cute pink kawaii girl' },
  { ref: '201b57b4da0bd9d1', label: 'green_pastel', prompt: 'cute green pastel anime girl' },
  { ref: '94ad2a0e52cfc4d1', label: 'twin_tail',    prompt: 'kawaii girl with pigtails' },
  { ref: 'bb2adcac96e12d93', label: 'long_blonde',  prompt: 'cute blonde anime girl' },
  { ref: 'f2b8ea9653543e19', label: 'frog_hood',    prompt: 'tiny character in frog hoodie' },
]
...
function faceFraming(grid, skinB) {
  let n=0
  for(let r=0;r<3;r++) for(const c of grid[r]) { if(!c) continue; if(bucket(c)!==skinB) n++ }
  return n/24
}
...
async function generateOnce(prompt) {
  const res = spawnSync('curl', ['-sS','-X','POST','http://localhost:3000/api/generate','-H','Content-Type: application/json','-d', JSON.stringify({ prompt })], { encoding: 'utf8' })
...
async function main() {
  ...
  const results = []
  console.log(`=== USER-STYLE PROMPTS: best-of-${RUNS} per prompt ===\n`)
  for (const p of PROMPTS) {
    ...
    for (let r = 0; r < RUNS; r++) {
      try {
        ...
        const { sim } = scoreAndBreakdown(refMetrics, genMetrics)
        const cand = { sim, localPath, intent, hairStyle: intent?.character?.hair_style ?? '?' }
        if (!best || sim > best.sim) best = cand
        console.log(`  ${p.label} run${r+1}: ${sim.toFixed(1)} [${cand.hairStyle}]`)
      } catch (e) { console.error(`  ${p.label} run${r+1} failed:`, e.message) }
    }
    if (best) {
      await writeFile(path.join(OUT_DIR, `${p.label}-best.png`), await readFile(best.localPath))
      results.push({ ...p, best })
  ...
  console.log(`\nAVG (user-style prompts): ${avg.toFixed(1)}`)
}
